...
export default {
    ...
    methods:{
        ...
        rollSkill: function () {
            ...
            if(this.numSkillRed > 0){
                var totalRedFailure = 0;
                var totalRedDisadvantage = 0;
                var totalRedDespair = 0;
                var redOutput = 'Red: (';
                for(var redi = 0; redi < this.numSkillRed; redi ++){
                    var redRoll = Math.floor(Math.random() * 12) +1;
                    switch(redRoll){
                        case 1: redOutput += 'blank';
                            break;
                        case 2: redOutput += '1F';
                            totalRedFailure += 1;
                            break;
                        ...
                        case 5: redOutput += '2F';
                            totalRedFailure += 2;
                            break;
                        case 6: redOutput += '1Da';
                            totalRedDisadvantage += 1;
                            break;
                        case 7: redOutput += '1Da';
                            totalRedDisadvantage += 1;
                            break;
                        case 8: redOutput += '1F 1Da';
                            totalRedFailure += 1;
                            totalRedDisadvantage += 1;
                            break;
                        case 9: redOutput += '1F 1Da';
                            totalRedFailure += 1;
                            totalRedDisadvantage += 1;
                            break;
                        case 10: redOutput += '2Da';
                            totalRedDisadvantage += 2;
                            break;
                        case 11: redOutput += '2Da';
                            totalRedDisadvantage += 2;
                            break;
                        case 12: redOutput += '1D';
                            totalRedDespair += 1;
                            break;
                    }
                    if(redi != this.numSkillRed -1){
                        redOutput += ', ';
                    }
                }
                redOutput += ')'
                if(totalRedDespair > 0){
                    redOutput += ' D:' + totalRedDespair;
                }
                if(totalRedFailure > 0){
                    redOutput += ' F:' + totalRedFailure;
                }
                if(totalRedDisadvantage > 0){
                    redOutput += ' Da:' + totalRedDisadvantage;
                }
                totalDespair += totalRedDespair;
                totalFailure += totalRedFailure;
                totalDisadvantage += totalRedDisadvantage;
                fullSkillRollResult.redResult = redOutput;
                //process rolls
            }

            if(this.numSkillPur > 0) {
                var totalPurFailure = 0;
                var totalPurDisadvantage = 0;
                var purOutput = 'Purple: (';
                for (var puri = 0; puri < this.numSkillPur; puri++) {
                    var purRoll = Math.floor(Math.random() * 8) + 1;
                    switch (purRoll) {
                        case 1:
                            purOutput += 'blank';
                            break;
                    ...
                    if (puri != this.numSkillPur - 1) {
                        purOutput += ', ';
                    }
                }
                purOutput += ')'
                if (totalPurFailure > 0) {
                    purOutput += ' F:' + totalPurFailure;
                }
                if (totalPurDisadvantage > 0) {
                    purOutput += ' Da:' + totalPurDisadvantage;
                }
                totalFailure += totalPurFailure;
                totalDisadvantage += totalPurDisadvantage;
                fullSkillRollResult.purResult = purOutput;
                //process rolls
            }

            if(this.numSkillBla > 0) {
                var totalBlaFailure = 0;
                var totalBlaDisadvantage = 0;
                var blaOutput = 'Black: (';
                for (var blai = 0; blai < this.numSkillBla; blai++) {
                    var blaRoll = Math.floor(Math.random() * 6) + 1;
                    switch (blaRoll) {
                        case 1:
                            blaOutput += 'blank';
                            break;
                    ...
                    if (blai != this.numSkillBla - 1) {
                        blaOutput += ', ';
                    }
                }
                blaOutput += ')'
                if (totalBlaFailure > 0) {
                    blaOutput += ' F:' + totalBlaFailure;
                }
                if (totalBlaDisadvantage > 0) {
                    blaOutput += ' Da:' + totalBlaDisadvantage;
                }
                totalFailure += totalBlaFailure;
                totalDisadvantage += totalBlaDisadvantage;
                fullSkillRollResult.blaResult = blaOutput;
                //process rolls
            }
            //more dice
            ...
            fullSkillRollResult.despairResult = totalDespair;
            fullSkillRollResult.failureResult = totalFailure;
            fullSkillRollResult.disadvantageResult = totalDisadvantage;
